...
\begin{frame}{Bayesian Cubature}
	...
	\vspace*{-4ex}
	The Bayesian approach for numerical analysis was popularized by \ocite{Dia88a}. 
...
\vspace*{-1ex}
\begin{align*}
\text{Define} \quad c_0 &= \int_{[0,1] \times [0,1]} C_{\vtheta} (\vx,\vt) \dvx \dvt, 
\\
...
0, \quad s^2(c_0 - \vc ^T \mCthetaInv \vc)
\biggr)
\quad % \\
\text{where} \; \vy = \bigl(f(\vx_i)\bigr)_{i=1}^n. 
\\
\text{Choosing} \; \hmu_n &= \underbrace{m(1 - \vone^T \mCthetaInv \vc)}_{w_0} + {(\underbrace{\mCthetaInv\vc}_{\vw})}^T \vy \; \text{makes error unbiased.} 
\end{align*}
\pause
If $\alert{m = 0}$, fixed, choosing $\vw = \mCthetaInv\vc \;$ makes error unbiased. Please note \alert{$m, s$ and $\vtheta$} needs to be inferred.
...
\err_{\MLE} & = 2.58 \; s_{\MLE} \; \sqrt{c_{0} - \vc^T\mCthetaInv\vc }
\end{align*}
\pause %\vspace{-3ex}
...
\begin{frame}
	...
	\vspace*{-2.0ex}
	\begin{align*}
	\textup{CV} &= \sum_{i=1}^n (y_i - \ty_i)^2 = \sum_{i=1}^n \left(\frac{\zeta_i }{a_{ii}} \right)^2, \quad \text{where}\; \bm{\zeta} = \mCthetaInv(\vy - m \vone), 
	\\
	& \qquad \qquad a_{ii} \; \text{are diagonal elems of} \; \mCthetaInv = \begin{pmatrix} a_{ii}  & \vA_{-i,i}^T \\  \vA_{-i,i} & \mA_{-i,-i}\end{pmatrix}
	\\
	\GCV &
	...
	\vspace{-2ex}
	\begin{align*}
	\vtheta_{\GCV} &= \argmin_{\vtheta} \left\{\log \left(  \vy^T \left[\mCtheta^{-2} - \frac{\mCtheta^{-2} \vone \vone^T \mCtheta^{-2}}{\vone^T \mCtheta^{-2} \vone}  \right] \vy \right)  
	...
	\intertext{The credible interval width, $\err_{\MLE}$, is given by}
	\err_{\GCV} & = 2.58 \; s_{\GCV} \; \sqrt{c_{0} - \vc^T\mCthetaInv\vc }
	\end{align*}
\end{frame}
...
\rbrace
\end{align}
where $\Delta$ is a random shift and $\bm{h}$ is a \emph{generalized Mahler integer} ($\infty$ digit expression) \smallcite{HicNie03a} also called \alert{generating vector}.
