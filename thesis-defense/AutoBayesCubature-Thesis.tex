	...
	% \uncover<1-2>
	{
		\begin{algorithm}[H]
			\begin{algorithmic}[1]
				...
				compute credible interval \alert{$\err_\CI$ } %\Comment{$\errn$ data driven error bound}
...
\begin{frame}
	{Shift invariant covariance kernels}
	\vspace*{-6ex}
	\begin{align*}
	\redroundmathbox{
		\displaystyle{
...
\frame{
	\frametitle{Shift invariant kernels}
	\vspace{-6ex}
	\begin{figure}[htp]
		\centering
...
\frame{
\frametitle{Multivariate normal probability: Lattice $d=2, r=2, \text{C2Sin}$ }
\vspace{-5ex}
\begin{figure}[htp]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
...
\frame{
	\frametitle{Multivariate normal probability: Sobol' $d=2, r=1$ }
	\vspace{-5ex}
	\begin{figure}[htp]
		\centering
		\begin{subfigure}[b]{0.48\textwidth}
		...
		\begin{subfigure}[b]{0.48\textwidth}
			\includegraphics[height=3.25cm]{../figures/MVN_guaranteed_time_Matern_d2_2019-Jun-29}
			\caption{\alert{Slow cubature} with Mat\'ern  kernel}
		\end{subfigure}	
		% \caption{Multivariate normal probability example using 1) Empirical Bayes, 2) GCV, 3) Full Bayes stopping criterion}
	\end{figure}
}
...
\frame{
\frametitle{Keister Integral: Lattice $d=4, r=2, \text{C1Sin}$}
\vspace{-5ex}
\begin{figure}[htp]
	\linespread{0.7}
    \centering
...
\frame{
	\frametitle{Keister Integral: Sobol' $d=4, r=2$}
	\vspace{-5ex}
	\begin{figure}[htp]
		\linespread{0.7}
		\centering
...
\frame{
	\frametitle{Option pricing: Lattice $d=12, r=1, \text{Baker}$}
	\vspace{-5ex}
	\begin{figure}[htp]
		\linespread{0.7}
		\centering
...
\frame{
	\frametitle{Option pricing: Sobol' $d=12, r=1$}
	\vspace{-5ex}
	\begin{figure}[htp]
		\linespread{0.7}
		\centering
...
\item Higher order digital nets and Walsh kernels could be used to achieve higher order of accuracy.
\item Steepest descent for paramter search.
\end{itemize}
\end{minipage}
}
...
$ (f - \beta_1 g_1 -, ... , - \beta_p g_p) $, assuming
\begin{align*}
f = \mathcal{GP} \left( \beta_0 + \beta_1 g_1 + , ... , + \beta_p g_p, s^2 \mCtheta  \right)
\end{align*}
\item \alert{Function approximation} : 
consider approximating a function $f$, given
\begin{align*}
