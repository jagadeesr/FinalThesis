...
% \\
% \text{If $\alert{m = 0}$ fixed, choosing} \quad \vw & = \mathsf{C}^{-1}\vc, \; \text{ makes error unbiased }
\end{align*}
...
%the transform {\redroundmathbox{ { \text{$\hat{\vz} = \mV^H \vz$ }  } } } for arbitrary $z$, can be done in \alert{$\Order( n \, log\, n) $}. 
...
, \quad
s^2_\MLE = \frac 1n \sum_{i=2}^n \frac{\abs{\ty_i}^2}{\lambda_i}, 
\quad
s^2_{\GCV}  =  \frac 1{n} \sum_{i=2}^n \frac{\abs{\ty_i}^2}{\lambda_i^2}  \left [ \sum_{i=1}^n \frac{1}{\lambda_i} \right]^{-1}
\\
\widehat{\sigma}^2_{\full}
&=
\frac{1}{n(n-1)} \sum_{i=2}^n \frac{\abs{\ty_i}^2}{\lambda_i} \,  \left(\frac{\lambda_1}{n}  - 1  \right), \quad \text{where}
\end{align*}
\vspace{-3ex}
\begin{align*}
{\tvy} &= (\ty_i)_{i=1}^{n} = \mV^T \vy ,
&   \vlambda &= ( \lambda_i )_{i=1}^{n} = \mV^T  \vC_1, \quad \text{where} \; \vC_1 =  \big( C(\vx_i, \vx_1) \big)_{i=1}^{n}
\end{align*}
\alert{$\Order(n \log n)$ operations to compute $\tvy$ and $\hat{\vlambda}$, So the $\vthetaMLE$}
}
...
\frame{
\frametitle{Computing the error bound $\err_{\CI}$ and $\hmu$ faster }
\vspace{-5ex}
...
% Lattice/Lattice_MVN_guaranteed_time_MLE_C2sin_d2_r2_2019-Jun-27
...
\item This work is to be published as a paper "Fast Automatic Bayesian Cubature Using Lattice Sampling by R. Jagadeeswaran, Fred J. Hickernell" (\alert{\url{https://arxiv.org/abs/1809.09803}})
\end{itemize}
\end{minipage}
}
...
\frame{\frametitle{Future work : Higher order nets and fast Walsh transform}
	
	\begin{itemize}
		\vspace{-3ex}
		\item
		Higher order nets and Walsh kernels could be used to achieve higher order or accuracy.
		% Using the established generalized concept of a \textit{Fast transform kernel}, we could use other kernels with suitable point sets to achieve similar or better performance and accuracy. 
		% \item
