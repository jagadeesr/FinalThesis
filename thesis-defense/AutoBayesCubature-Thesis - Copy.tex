...
\date[]{Thesis Defense \textbullet\ Oct 10, 2019}
  ...
  \Procedure{AutoBayesCubature}{$f$,$\errtol$} 
    %\Comment{Integrate within the error tolerance \alert{$\errtol$}}
      ...
      \While{$\errn > \varepsilon$ and $n \le n_{\textup{max}}$} %\Comment{Iterate till error tolerance is met}
        ...
        compute error bound \alert{$\errn$ } %\Comment{$\errn$ data driven error bound}
...
\begin{frame}
\frametitle{Parameter estimation - Cross validation}
\vspace*{-6ex}
...
1 - \eta_\ell^r  \frac{(2 \pi \sqrt{-1})^{r}}{r!} B_{r}( |{x_l-t_l}| ), \quad r \in 2\mathbb{N}, \quad \eta_\ell > 0, \quad \vtheta = (\bm{\eta}, r) }
}
\end{align*}
\vspace*{-0ex} 
\pause
...
\quad
\text{then} \; \rmC = \mC - \vone \vone^T, \quad \text{and} \; \rmC = \mV \rmLambda \mV^H
\end{align*}
where
\vspace{-3ex}
\begin{align*}
