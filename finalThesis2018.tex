...
\begin{table} % MATLAB Driver: KeisterCubatureExampleBayes.m
\centering
\caption{Average performance of cubatures with automatic stopping 
		criteria for estimating the integrals in \eqref{eqn:keister_integral}
...
\begin{table} % MATLAB Driver: MVNCubatureExampleBayes.m
	\centering
	\caption{Average performance of cubatures with automatic stopping 
		criteria for estimating the $d=20$ Multi-variate Normal in \eqref{eqn:fGenzdef}
		for $1000$ independent runs with $\varepsilon=10^{-3}$. These results can be conditionally reproduced with the
		script, \texttt{MVNCubatureExampleBayes.m}, in GAIL. 
		\label{tab3}}	   
	\vspace{-2ex}
	$
	%\arraycolsep=1.4pt\def\arraystretch{0.9}
