...
Searching for $\vtheta_\MLE$ as defined in \eqref{eqn:thetaMLE} is not very efficient. Using the derivative of $l(s,m,\vtheta | \vy)$, We can apply gradient descent to find the optimal value faster. Let us define the objective function for the same purpose by excluding the negative sign, which modifies the problem as minimization
\begin{align*}
\mathcal{L}(\vtheta | \vy)
...
\subsection{Kernel derivative using fast transform}
...
\end{align*}
where $\bar{\mLambda}_{(j)} = \diag(\bar{\vlambda}_{(j)})$ and $\bar{\vlambda}_{(j)} = \frac{\partial \vlambda}{\partial \theta_j} = \left( \frac{\partial \lambda_i}{\partial \theta_j} \right)_{i=1}^n $.
Also, note that $\bar{\vlambda}_{(j)} = \mV^H \bar{\vC_1}_{(j)}$ computed using the fast transform,  where $\bar{\vC}_{1(j)}$ denotes the first row of the gram matrix after taking derivative, i.e.
\begin{align*}
\bar{\vC}_{1{(j)}} = \left(\frac{\partial }{\partial{\theta}_i} C_\vtheta(\vx_1,\vx_i) \right)_{i=1}^n.
\end{align*}
First, let's define the objective function from \eqref{eqn_MLE_loss_func_optimized_2},
\begin{align*}
\mathcal{L}(\vtheta | \vy) &= 
\frac{1}{n}  \log(\det\, \mC) + \log\left((\vy-m_\MLE\vone)^T\mCInv(\vy-m_\MLE\vone) \right)
\\ &= \mathcal{L}_{\abs{\mC}} + \mathcal{L}_{\vy}
\end{align*}
Now, let's take the derivative
\begin{align*}
\frac{\partial}{\partial \theta_i} \mathcal{L}(\vtheta | \vy)
&=  \frac{\partial}{\partial \theta_i} \mathcal{L}_{\abs{\mC}} + \frac{\partial}{\partial \theta_i} \mathcal{L}_{\vy}
\end{align*}
Let's simplify the individual terms
\begin{align*}
\frac{\partial}{\partial \theta_i} \mathcal{L}_{\abs{\mC}} &= \frac{\partial}{\partial \theta_i}  \frac{1}{n} \log(\det\, \mC) 
\\ & = \trace{\left( \mCInv \frac{\partial \mC}{\partial \vtheta_i} \right)}
 =
\trace{\left(
\frac 1n \mV {\mLambda}^{-1} \mV^H
\frac 1n \mV \overline{\mLambda} \mV^H
\right)}
\\
& =
\trace{\left(
	\frac{1}{n} \mV {\mLambda}^{-1}  \overline{\mLambda} \mV^H
	\right)}, \quad \text{where we used } \; \mV^H \mV = n
\\
& =
\trace{\left(
	\frac{1}{n} \mV \;
	\diag\left( \frac{\overline{\lambda}_j}{\lambda_j} \right)_{j=1}^n \mV^H
	\right)}
= \sum_{j=1}^{n} \frac{\overline{\lambda}_j}{\lambda_j}
\end{align*}
...
\begin{align*}
\frac{\partial}{\partial \gamma_i} C_\vtheta(\vt, \vx) 
& =
...
\sum_{j=1}^d 
\frac{
	\biggl(1 - (-1)^{r} \gamma B_{2r}( |{x_j-t_j}|  - 1) \biggr)
}{
	1 - (-1)^{r} \gamma B_{2r}( |{x_j-t_j}| ) 
}
\\
...
Now we can implement the steepest descent search introduced in \eqref{Dong2017a}
...
\Chapter{Sobol Points and Walsh Kernels}
...
\paragraph{Consider the case $\alpha=2.$}
\begin{align}
\nonumber
\omega_{b,2} &= 
...
\paragraph{Non-informative prior}
...
\paragraph{TODO: How to interpret as t-distribution?}




\paragraph{Generic prior}
...
m_\MLE=\frac{(\vone^T \mCInv \vy)}{\vone^T\mCInv\vone}
\end{align*}
\paragraph{TODO: How to integrate further?}
...
\begin{itemize}
	\item Optimal kernel order selection for the integrand
\end{itemize}
