...
\begin{align*}
\omega_2(x) &= \sum_{k=1}^\infty r_2(k) \wal_k(x)
\\
&= s_1(x) + \tilde{s}_2(x)
\end{align*}
...
\begin{align*}
s_1(x) &= \sum_{\substack{k=1 \\ k = 2^{a_1}}}^\infty r_2(k) \wal_k(x)
\\
&= \sum_{\substack{k=1; k = 2^{a_1}}}^\infty 2^{(-a_1 + 1)} \exp(\pi \sqrt{-1} \xi_{a_1 + 1})
\\
&= \sum_{\substack{a_1=0}}^\infty 2^{-(a_1 + 1)} \exp(\pi \sqrt{-1} \xi_{a_1 + 1})
\\
&= \sum_{\substack{a_1=1}}^\infty 2^{(-a_1)} \exp(\pi \sqrt{-1} \xi_{a_1 }), \qquad \text{where} \; \xi_a \in \{0, 1\}
\\
&= \sum_{\substack{a_1=1}}^\infty 2^{(-a_1)} (-1)^{\xi_{a_1 }}
\quad = \quad \sum_{\substack{a=1}}^\infty 2^{(-a)} (-1)^{\xi_{a }}
\\
&= \sum_{\substack{a=1}}^\infty 2^{-a} +  \sum_{\substack{a=1}}^\infty 2^{-a}( (-1)^{\xi_{a }} - 1)
\\
&= 1 +  \sum_{\substack{a=1}}^\infty 2^{-a}( (-1)^{\xi_{a }} - 1)
\\
&= 1 +  \sum_{\substack{a=1}}^\infty 2^{-a} (-2) [\xi_a = 1]
\\
&= 1 - 2  \sum_{\substack{a=1}}^\infty 2^{-a} [\xi_a = 1]
\\
&= 1 - 2  \sum_{\substack{j=1}}^{\#x} 2^{-a_j}, \quad \text{using} \; x = \sum_{\substack{j=1}}^{\#x} 2^{-a_j}
\\
&= 1 - 2  x
\end{align*}
...
\begin{align*}
\tilde{s}_2(x) &= \sum_{\substack{k=1; k = 2^{a_1} + 2^{a_2} + k' }}^\infty r_2(k) \wal_k(x)
\end{align*}
where $k = 2^{a_1} + 2^{a_2} + k', \; k' \in \{0, \dots 2^{a_2}-1 \}$
\begin{align*}
\tilde{s}_2(x) &= \sum_{\substack{k=1; k = 2^{a_1} + 2^{a_2} + k'}}^\infty 
2^{-(a_1+1)} (-1)^{\xi_{a_1+1}} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} \wal_{k'}(x)
\\
&= \sum_{a_1=1}^\infty 2^{-(a_1+1)} (-1)^{\xi_{a_1+1}} 
\sum_{a_2=0}^{a_1-1} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} \sum_{k'=0}^{2^{a_2}-1} \wal_{k'}(x)
\\
&= \sum_{a_2=0}^{\infty} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} \sum_{k'=0}^{2^{a_2}-1} \wal_{k'}(x)
\sum_{a_1=a_2+1}^\infty  2^{-(a_1+1)} (-1)^{\xi_{a_1+1}} 
\\
&= \sum_{a_2=0}^{\infty} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} 2^{a_2} [x < 2^{-a_2}]
\sum_{a_1=a_2+1}^\infty  2^{-(a_1+1)} (-1)^{\xi_{a_1+1}} 
\\
&= \sum_{a_2=0}^{\infty} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} 2^{a_2} [x < 2^{-a_2}]
2^{-(a_2+1)} \sum_{a_1=1}^\infty  2^{-(a_1)} (-1)^{\xi_{a_1}} 
\end{align*}
