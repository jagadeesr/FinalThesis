...
This explained using the following example. We have chosen a smooth and periodic integrand $f_{nice}(\vx) = \exp(\sum_{j=1}^{d} \cos(2\pi x_j))$ and another integrand $f_{real}(\vx) = f_{nice} + f_{noise}$ which satisfies $f_{nice}(\vx_i) = f_{real}(\vx_i) $ at the sampling nodes $\{ \vx_i\}_{i=1}^n$. Where the function $f_{noise}(\vx) = a(1 - \exp(2\pi i \sqrt{-1} \vx^T \vzeta))$ chosen to be zero at the sampling nodes $\{ \vx_i\}_{i=1}^n$, where $a \in \mathbb{R}$ is some constant, $\vzeta \in \mathbb{R}^d$ is some $d$-dimensional vector belonging to the dual space of the lattice nodes $\{ \vx_i\}_{i=1}^n$ chosen to sample the integrand.
...
As shown in \figref{fig:cone_bayes_functions}, sampled function values $\{f(\vx_i)\}_{i=1}^n$ from a smooth integrand $f_{true}$ are shows as dots. One can imaging these sample $\{f(\vx_i)\}_{i=1}^n$ were obtained from $f_{nice}$, a moderately smoother function or from $f_{peaky}$ a highly oscillating function.

When using $n=16$ points and $r=1$ shift-invariant kernel, we get the posterior distribution of $\mu$ as shown in \figref{fig:cone_bayes_posterior}. The true integral value is shown as $\mu_{true}$ which is at the center of the plot. The integral of the peaky function $f_{peaky}$ almost lies outside of the 99\% of the confidence interval given by \eqref{eqn_prob_CI_MLE}, whereas the $\mu_{nice}$ falls within.



\Section{The automatic Bayesian cubature algorithm} \label{sec:bayes_cubature_algo}
...
\end{align*}
The goal is to find derivative of the objective function. First, let's define the objective function from \eqref{eqn_MLE_loss_func_optimized_2},
\begin{align*}
\mathcal{L}(\vtheta | \vy) &= 
...
\Subsection{Derivative of the kernel}
...
\Subsubsection{When $\gamma_j$ fixed per dimension $j$}
...
\Chapter{Sobol Nets and Walsh Kernels}\label{sec:sobol_walsh}
...
We demonstrate an alternate approach to formulate fast transform using matching kernel and point sets in this section. 
...
Digital sequences are defined in terms of digitwise addition. Let $b$ be a prime number. Digitwise addition, $\oplus$, negation $\ominus$, are defined in terms of $b$-ary expansions of points in $[0, 1)^d]$,
\begin{align*}
...
Like Fourier transform naturally occurred with Lattice points (Section \ref{sec:shift_invar_kern}), Walsh transform happens for the nets. Define $\naturals:=\lbrace1,2,\dots \rbrace$ and $\naturals_0 : \lbrace0,1,2,\dots \rbrace$.
...
\end{align*}
where $r$ is kernel order, $\frac 1p + \frac 1q = 1$ are H\"older inequality coefficients. We have explicit expression for $\omega_{r}$ \cite{Nuyens2013} in case of order $r=1$, $b=2$ and Hilbert space setup $q=2$
\begin{align}
...
The qualifier ``higher order" emphasizes that quasi-Monte Carlo rules based on these point sets can achieve convergence rates faster than $\Order(N^{-1})$.
...
Another approach to formulate fast transform using matching kernel and point sets is presented in this section. Interlaced digital nets and smoother kernels are paired. Higher order digital nets with matching smoother kernels are expected to provide higher order error convergence $\Order(N^{-\alpha + \epsilon})$ for $\alpha \ge 1$ and $\epsilon > 0$ .

\Section{Higher order digital nets}
Nets were developed to provide deterministic sample points for quasi-Monte Carlo rules \cite{Nie05a}.
The $(t,m, s)$-nets in base $b$ introduced by Niederreiter are such point sets consist of $b^m$ points in $[0, 1)^s$, whose quality is governed by $t$, in particular, lower values of $t$ correspond to $(t,m, s)$-nets of higher quality \cite{Bald10a}.
Digital $(t,m, s)$-nets are a special case of $(t,m, s)$-nets, constructed using matrix-vector multiplications over finite fields.

Higher order digital nets are a modified $(t,m,s)$-nets, introduced in \cite{Dic08a} can be used to numerically integrate smoother functions which are not necessarily periodic, but have square integrable mixed partial derivatives of order $\alpha$, at a rate of $\Order(N^{-\alpha})$ multiplied by a power of a $\log N$ factor using rules corresponding to the modified $(t,m, s)$-nets.
The qualifier ``higher order" emphasizes that quasi-Monte Carlo rules based on these point sets can achieve convergence rates faster than $\Order(N^{-1})$.

Higher order digital nets are constructed using matrix-vector multiplications over finite
fields. We use generating matrices from the magic point shop \cite{NuyMagic}.
...
$h = \sum_{i \ge 1} h_i b^{-i} = (0.h_1 h_2 \dots)_b, [\vec{h}] =  (h_1,\dots h_n)^T$,
with $n$ at least as large as the number of digits to represent $x$ or $h$.
Multivariate wlash functions are defined as the product of the one-dimensional walsh functions
\begin{align*}
\textup{wal}_{b,\vh} (\vx) := \prod_{j=1}^s \textup{wal}_{b,h_j} (x_j
)
\end{align*}

\Section{Higher order Walsh space}
...
\begin{align*}
\omega_1(\vx) &= \frac 16 - 2^{\lfloor \log_2 \vx\rfloor -1 },\\
\omega_2(\vx) &= s_1(\vx) + \tilde{s}_2(\vx),\\
\omega_3(\vx) &= s_1(\vx) + s_2(\vx) + \tilde{s}_3(\vx)
\end{align*}
where
\begin{align*}
s_1(\vx) &= 1-2\vx,\\
s_2(\vx) &= 1/3 - 2(1-\vx) \vx, \\
\tilde{s}_2(\vx) &= (1- 5 t_1)/2 - (a_1 - 2)\vx, \\
\tilde{s}_3(\vx) &= (1- 43 t_2)/18 + (5t_1 - 1)\vx +(a_1 - 2)\vx^2.
\end{align*}
with, for $0 < \vx < 1$,
\begin{align*}
a_1 = -\lfloor \log_2 \vx\rfloor, \quad t_1 = 2^{-a_1}, \quad t_2 = 2^{-2 a_1},
\end{align*}
and $a_1=0, t_1=0$ and $t_2=0$ when $\vx=0$.


\Subsection{The worst-case error}
Define the worst-case error of numerical integration using a cubature rule $Q$ for functions in a Hilbert space $\mathcal{F}$ by
\begin{align*}
e(Q,\mathcal{F}) := \sup_{f \in \mathcal{F}, \norm[\mathcal{F}]{f} \le 1 } 
\abs{I(f) - Q(f)}
\end{align*}
We now choose cubature rule $Q$ to be a quasi-Monte Carlo using a higher order digital as its node set and denote it by $Q^{\text{net}}$
\begin{align*}
\abs{I(f) - Q^{\text{net}}(f)} = \abs{\sum_{\vzero \ne \vk \in \mathcal{D}} \widehat{f_{\vk}}} \le \sum_{\vzero \ne \vk \in \mathcal{D}} \abs{ \widehat{f_{\vk}}}
\end{align*}


Define $\mathcal{W}_{\alpha,d,\vgamma}$, the function space consisting of all functions $f \in L_2([0,1]^d)$ for which $\norm[\mathcal{W}_{\alpha,d,\vgamma}]{f} < \infty$, where 
\begin{align*}
\norm[\mathcal{W}_{\alpha,d,\vgamma}]{f}^2 := \sum_{\vk \in \naturals^d} 
\left( \frac{\abs{\widehat{f_{\vk}}}}{\vr_{\alpha}(\vgamma,\vk)} \right)^2
\end{align*}
where $\widehat{f_{\vk}}$ are the walsh coefficients of $f$ which satisfy
\begin{align*}
\abs{\widehat{f_{\vk}}}^2 \le \norm[\mathcal{W}_{\alpha,d,\vgamma}]{f}  
\left( {\vr_{\alpha}(\vgamma,\vk)} \right)^2 \qquad \forall \vk \in \naturals^d_0
\end{align*}
Given a fixed integer $\alpha > 1$ and a fixed sequence of positive weights $\vgamma = \{ \gamma_1, \gamma_2, \dots \}$, the decay function $\vr_{\alpha}(\vgamma,\vk)$ is defined as  
\begin{align*}
\vr^2_{\alpha}(\vgamma,\vk) = \prod_{j\in u} \gamma^2_j r^2_\alpha(k_j), \qquad r_\alpha(k) := 2^{- \sum_{i=1}^{\min(\#k,\alpha)} (a_i + 1)}
\end{align*}
where for $\vk = (k_1, \dots, k_d), k = \sum_{i=1}^{\#k} k_{a_i} b^{a_i}$, we set $u = \{1\le j \le d: k_j \neq 0 \}$ and $\#k$ is the number of non-zero binary digits of integer $k$.

\begin{align*}
\zeta(\vx) = \sum_{\vk \in \naturals} \vr_{\alpha} (\vgamma,\vk) \wal_{\vk} (\vx)
\end{align*}



\Subsection{Calculating the infinite sum}
For practical usage with a computer, we can not have infinite sum. In this section we show how to calculate in finite terms. If $x$ can be represented using $n$ digits in base $b$, then $\omega_{\alpha}(x)$ can be computed in $\Order(\alpha n)$ operations \cite{Bald12a}. Specifically, we will compute explicit forms for $b=2$ and $x \in [0,1)$.]

\begin{align}
\label{omega_func_split_1}
\omega_{\alpha}(\vx) &:= \sum_{\vk \in \naturals} \vr^2_{\alpha} (\vk) \wal_{\vk} (\vx)
\\
&= \sum^\infty_{\substack{\vk \in \naturals \\ k= \sum_{i=1}^{\#k}k_{a_i} b^{a_i} }} 
\prod_{i=1}^{\min(\alpha, \#k)} b^{-2(a_i + 1)} \exp(2\pi i k_{a_i} \xi_{a_i + 1}/b)
\underbrace{\prod_{i=\alpha + 1}^{\#k}  \exp(2\pi i k_{a_i} \xi_{a_i + 1}/b)}_{\wal_{k'}(x)}
\end{align}
We used the definition $k= \sum_{i=1}^{\#k}k_{a_i} b^{a_i}$ with $a_1 > \dots a_{\#k}$, i.e., $k_{a_1}$ is the most significant digit of $k$ in base $b$. The second product in \ref{omega_func_split_1} can be seen as $\wal_{k'}(x)$ where $k' = k-\sum_{i=1}^{\min(\alpha, \#k)} k_{a_i}b^{a_i}$. The key technique used is to interpret the infinite sum in \ref{omega_func_split_1} as multiple sums over all possible digit expansions for all $k$'s with $r$ digits for $r\ge 1$.

\begin{align*}
\sum^\infty_{\substack{\vk \in \naturals \\ k= \sum_{i=1}^{\#k}k_{a_i} b^{a_i} }} 
G(k,x) = \sum_{r=1}^\infty 
\underbrace{ \sum_{a_1 = r-1}^\infty \dots \sum_{a_r=0}^{a_{r-1}-1}}_{r \text{sums s.t.} \infty > a_1 \dots a_r \ge 0}
\underbrace{ \sum_{k_{a_1} = 1}^{b-1} \dots \sum_{k_{a_r}=1}^{b-1}}_{r \text{independent sums}}
G(k_{a_i} b^{a_i},x)
\end{align*}

where $G(k,x) = r_\alpha(k) \wal_k(x)$. to simplify the notation and reveal the structure for further simplification, define the \emph{triangular sum operator}
\begin{align*}
T_m^M(r)(g) := \sum_{a_1 = m+r-1}^M \sum_{a_2 = m+r-2}^{a_1 -1} \dots \sum_{a_r=m}^{a_{r-1}-1} g(a_1,\dots a_r),
\end{align*}
where $T_m^M(0)(g) := g$, i.e., no sums taken. Next, define the \emph{concatenation} of two such operators as putting the sums next to each other:
\begin{align*}
\left( T_m^M(t) T_{m'}^{M'}(r-t)\right) (g) :=
\sum_{a_1=m+t-1}^M \dots \sum_{a_t=m}^{a_{t-1}-1}
\sum_{a_{t+1}=m'+(r-t)-1}^{M'} \dots \sum_{a_r=m'}^{a_{r-1}-1}
g(a_1,\dots,a_r),
\end{align*}
...
\begin{align*}
\omega_{b,\alpha} := \sum_{\kappa=1}^\infty r_{b,\alpha} \wal_{b,\kappa} (x)
\end{align*}
for a given base $b$ and number $x \in [0,1)$. Given an integer $\kappa$, consider its base $b$ expansion 
\begin{align*}
\kappa = \sum_{i=0}^\infty \kappa_i b^i, \quad k_i \in \integers_b := \{0,1,\dots,b-1\}
\end{align*}
We are interested in the non-zero digits of $\kappa$ in base $b$. Denote the number of non-zero digits of $\kappa$ by $\#\kappa$.
\begin{align*}
\kappa = \sum_{i=1}^{\#\kappa} \kappa_{a_i} b^{a_i}, \quad k_{a_i} \in \integers_b \backslash
 \{0\}, \quad a_1 > a_2 \dots a_{\#\kappa} \ge 0
\end{align*}
Now, define the function $r_{b,\alpha}(\kappa)$  
\begin{align*}
r_{b,\alpha}(\kappa) := b^{-\sum_{i=1}^{\min(\#\kappa,\alpha)} (a_i+1)} = 
\begin{cases*}
1, & if $\kappa$ = 0, \\
b^{-((a_1+1)+\dots + (a_{\min(\#\kappa,\alpha)+1}))}, & for $\kappa$ as given above
\end{cases*}
\end{align*}
Note that the function $r_{b,\alpha}(\kappa)$  only depends on the $\alpha$ most-significant base-$b$ digit locations of $\kappa$, but in a such a way their digit values do not matter

Similarly, assume $x \in [0,1)$ has a base-$b$ expansion
\begin{align*}
x = \sum_{i=1}^{\infty} x_i b^{-i} = \sum_{i=1}^{\#x} x_{a_i} b^{-a_i}, 
\quad 1 \le a_1 < a_2 < \dots < a_{\#x}, 
\quad x_i \in \integers_b, \quad x_{a_i} \in \integers_b \backslash \{0\}
\end{align*}
where $-a_1$ is the most significant power of $b$. For non-zero $x$, we find $a_1 = - \lfloor \log_b(x)\rfloor$


\Subsubsection{Consider the case $\alpha=2.$}
\begin{align}
\nonumber
\omega_{b,2} &= 
\sum_{l=0}^{\infty} \sum_{h_l=1}^{b-1} r_{b,2} (h_l b^l) \wal_{b,h_l b^l} (x) \\
\nonumber
& \qquad + 
\sum_{l=0}^{\infty} \sum_{h_l=1}^{b-1} 
\sum_{l'=0}^{l-1} \sum_{h_{l'}=1}^{b-1} \sum_{h'=0}^{b^{l'}-1} 
r_{b,2} (h_l b^l + h_{l'} b^{l'} + h') \wal_{b, h_l b^l + h_{l'} b^{l'} + h'} (x) \\
&= s_1(x) + \tilde{s}_2(x)
\end{align}
For $b=2$ it gets much simpler
\begin{align*}
\omega_{b,2} &= 
\sum_{l=0}^{\infty} r_{b,2} (b^l) \wal_{b, b^l} (x)  + 
\sum_{l=0}^{\infty}  
\sum_{l'=0}^{l-1} \sum_{h'=0}^{b^{l'}-1} 
r_{b,2} (b^l + b^{l'} + h') \wal_{b, b^l + b^{l'} + h'} (x) \\
&= 
\sum_{l=0}^{\infty} 2^{-(l+1)} \exp(2\pi i x_{l+1}/2)   + 
\sum_{l=0}^{\infty}  
\sum_{l'=0}^{l-1} \sum_{h'=0}^{b^{l'}-1} 
r_{b,2} (b^l + b^{l'} + h') \wal_{b, b^l + b^{l'} + h'} (x) 
\end{align*}
...
Note that
\begin{align*}
\sum_{h_l=1}^{b-1} \wal_{b, h_l b^l} (x) = \exp(2\pi i x_{l+1}/2) = (-1)^{x_{l+1}}
\end{align*}

Let's start with $s_1$,
\begin{align*}
s_1(x) &= \sum_{l=0}^{\infty} r_{b,2} (b^l) \wal_{b, b^l} (x) 
\\
&= \sum_{l=0}^\infty 2^{-(l + 1)} \exp(2\pi \sqrt{-1} x_{l + 1}/2)
\\
&= \sum_{l=1}^\infty 2^{(-l)} \exp(\pi \sqrt{-1} x_{l }), \qquad \text{where} \; x_l \in \{0, 1\}
\\
&= \sum_{l=1}^\infty 2^{(-l)} (-1)^{x_{l }}
\\
&= \sum_{l=1}^\infty 2^{-l} +  \sum_{l=1}^\infty 2^{-l}( (-1)^{x_{l }} - 1)
\\
&= 1 +  \sum_{l=1}^\infty 2^{-l}( (-1)^{x_{l }} - 1)
\\
&= 1 +  \sum_{l=1}^\infty 2^{-l} (-2) [x_l = 1]
\\
&= 1 - 2  \sum_{l=1}^\infty 2^{-l} [x_l = 1]
\\
&= 1 - 2  \sum_{j=1}^{\#x} 2^{-a_j}, \quad \text{using} \; x = \sum_{j=1}^{\#x} 2^{-a_j}
\\
&= 1 - 2  x
\end{align*}


Similarly, we can solve $\tilde{s}_2$
\begin{align*}
\tilde{s}_2(x) &= \sum_{\substack{k=1; k = 2^{l} + 2^{l'} + k' }}^\infty r_2(k) \wal_k(x)
\end{align*}
where $k = 2^{l} + 2^{l'} + k', \; k' \in \{0, \dots 2^{l'}-1 \}$
\begin{align*}
\tilde{s}_2(x) &= \sum_{\substack{k=1; k = 2^{l} + 2^{l'} + k'}}^\infty 
2^{-(l+1)} (-1)^{x_{l+1}} 2^{-(l'+1)} (-1)^{x_{l'+1}} \wal_{k'}(x)
\\
&= \sum_{l=1}^\infty 2^{-(l+1)} (-1)^{x_{l+1}} 
\sum_{l'=0}^{l-1} 2^{-(l'+1)} (-1)^{x_{l'+1}} \sum_{k'=0}^{2^{l'}-1} \wal_{k'}(x)
\\
&= \sum_{l'=0}^{\infty} 2^{-(l'+1)} (-1)^{x_{l'+1}} \sum_{k'=0}^{2^{l'}-1} \wal_{k'}(x)
\sum_{l=l'+1}^\infty  2^{-(l+1)} (-1)^{x_{l+1}} 
\end{align*}

Using the character property or by the definition of $\wal$ function
\begin{align*}
\sum_{k=0}^{b^{l}-1} \wal_{k}(x) &= 
\sum_{k_0=0}^{b-1} \sum_{k_1=0}^{b-1} \dots \sum_{k_{l-1}=0}^{b-1}
\exp(2\pi i (x_1 k_0 + x_2 k_1 + \dots + x_{l} k_{l-1})/b) \\
&= 
\sum_{k_0=0}^{b-1} \exp(2\pi i (x_1 k_0)/b)
\sum_{k_1=0}^{b-1} \exp(2\pi i (x_2 k_1)/b)
\dots 
\sum_{k_{l-1}=0}^{b-1} \exp(2\pi i (x_{l} k_{l-1})/b)
\end{align*}
For $b=2$
\begin{align*}
\sum_{k=0}^{b^{l}-1} \wal_{k}(x) 
&= 
\sum_{k_0=0}^{1} \exp(\pi i (x_1 k_0))
\sum_{k_1=0}^{1} \exp(\pi i (x_2 k_1))
\dots 
\sum_{k_{l-1}=0}^{1} \exp(\pi i (x_{l} k_{l-1}))
\\
&= 
\sum_{k_0=0}^{1} (-1)^{x_1 k_0}
\sum_{k_1=0}^{1} (-1)^{x_2 k_1}
\dots 
\sum_{k_{l-1}=0}^{1} (-1)^{x_{l} k_{l-1}}
\\
&= 2^{l} \times [x < 2^{-l}] 
\end{align*}
Continuing with $\tilde{s}_2$
\begin{align*}
\tilde{s}_2(x) &= \sum_{l'=0}^{\infty} 2^{-(l'+1)} (-1)^{x_{l'+1}} 2^{l'} [x < 2^{-l'}]
\sum_{l=l'+1}^\infty  2^{-(l+1)} (-1)^{x_{l+1}} 
\end{align*}
Now suppose $x=0$, then
\begin{align*}
\tilde{s}_2(0) &= \sum_{l'=0}^{\infty} 2^{-(l'+1)} 2^{l'} \sum_{l=l'+1}^\infty  2^{-(l+1)} 
\\
&= \sum_{l'=0}^{\infty} 2^{-(l'+1)}  2^{l'}  2^{-l'}  \sum_{l=1}^\infty  2^{-(l+1)} 
\\
&= \sum_{l'=0}^{\infty} 2^{-(l'+1)}   2^{-1} \sum_{l=1}^\infty  2^{-l} 
\\
&= \sum_{l'=0}^{\infty} 2^{-(l'+1)}   2^{-1} 
\\
&= 2^{-1}   \sum_{l'=1}^{\infty} 2^{-l'}   
\\
&= 2^{-1}   
\end{align*}
Now suppose $x = x_{a_1}b^{-a_1} \ne 0$


\begin{align*}
{s}_2(x) &= \sum_{\substack{k=1 \\ 
		k = 2^{a_1} + 2^{a_2}}}^\infty 
2^{-(a_1+1)} (-1)^{\xi_{a_1+1}} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} 
\\
&= \sum_{a_1=1}^\infty 2^{-(a_1+1)} (-1)^{\xi_{a_1+1}} 
\sum_{a_2=0}^{a_1-1} 2^{-(a_2+1)} (-1)^{\xi_{a_2+1}} 
\\
&= \sum_{a_1=2}^\infty 2^{-(a_1)} (-1)^{\xi_{a_1}} 
\sum_{a_2=1}^{a_1-1} 2^{-(a_2)} (-1)^{\xi_{a_2}}, \quad \text{index shifted by 1}
\\
&= \sum_{a_2=1}^{\infty} 2^{-(a_2)} (-1)^{\xi_{a_2}} 
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)} (-1)^{\xi_{a_1}}
\\
&= \left( 
\sum_{a_2=1}^{\infty} 2^{-(a_2)} (-1)^{\xi_{a_2}} [\xi_{a_2}=0] 
+
\sum_{a_2=1}^{\infty} 2^{-(a_2)} (-1)^{\xi_{a_2}} [\xi_{a_2}\neq 0] \right)
\\
& \qquad
\left(
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)} (-1)^{\xi_{a_1}} [\xi_{a_1}=0] 
+
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)} (-1)^{\xi_{a_1}} [\xi_{a_1}\neq 0]
\right)
\\
&= 
\left( 
...
\begin{align*}
&= \sum_{a_2=1}^{\infty} 2^{-(a_2)} (-1)^{\xi_{a_2}} 
2^{-a_2} \sum_{a_1=1}^\infty  2^{-(a_1)} (-1)^{\xi_{a_1}} 
\\
&= \sum_{a_2=1}^{\infty} 2^{-(2a_2)} (-1)^{\xi_{a_2}} 
(1-2x)
\\
&= (1-2x) 
\left( \sum_{a_2=1}^{\infty} 2^{-(2a_2)} (-1)^{\xi_{a_2}}[\xi_{a_2}= 0] 
+ 
\sum_{a_2=1}^{\infty} 2^{-(2a_2)} (-1)^{\xi_{a_2}}[\xi_{a_2}\neq 0] \right)
\\
&= (1-2x) 
\left( \sum_{a_2=1}^{\infty} 2^{-(2a_2)}
+ 
2 \sum_{a_2=1}^{\infty} 2^{-(2a_2)} (-1)^{\xi_{a_2}}[\xi_{a_2}\neq 0] \right)
\\
&= (1-2x) 
\left( \frac{1}{3}
+ 
2 \sum_{j=1}^{\#x} 2^{-(2a_2)}  \right)
\end{align*}

\clearpage

Simplify the first part in ${s}_2$
\begin{align*}
&
\sum_{a_2=1}^{\infty} 2^{-(a_2)} (-1)^{\xi_{a_2}} [\xi_{a_2}=0]	
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)} (-1)^{\xi_{a_1}} [\xi_{a_1}=0]
\\
&=
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  [\xi_{a_2}=0]	
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)}  [\xi_{a_1}=0]
\\
&=
\sum_{a_2=1}^{\infty} 2^{-(a_2)} [\xi_{a_2}=0]	
\left(
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)}  -
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)} [\xi_{a_1} \neq 0]
\right)
\\
&=
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  [\xi_{a_2}=0]	
...
\\
&=
\sum_{a_2=1}^{\infty} \left( 2^{-(a_2)} [\xi_{a_2}=0] \right) 2^{-(a_2)} 
-
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  [\xi_{a_2}=0]	
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)} [\xi_{a_1} \neq 0]
\\
&=
\left(
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  -
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  [\xi_{a_2} \neq 0]	
\right)
 2^{-(a_2)} 
\\ & \quad -
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  [\xi_{a_2}=0]	
...
\\
&=
\sum_{a_2=1}^{\infty} 2^{-(2a_2)} 
  -
\sum_{a_2=1}^{\infty} 2^{-(2a_2)}  [\xi_{a_2} \neq 0]	
\\ & \quad -
\sum_{a_2=1}^{\infty} 2^{-(a_2)}  [\xi_{a_2}=0]	
\sum_{a_1=a_2+1}^\infty  2^{-(a_1)}  [\xi_{a_1} \neq 0]
\\
...
The shift-invariant covariance kernels underlying our Bayesian cubature  assume that the integrand has a degree of periodicity, with the smoothness assumed depending on the smoothness of the kernel.  
...
\begin{figure}
	...
	\caption[OptPrice guaranteed : GCV]{Option pricing using the  GCV stopping criterion.} \label{fig:optprice-guaranteed-GCV}
\end{figure}
...
\begin{figure}
	\centering
	%d=3 problem transformed into d=2
	\includegraphics[width=0.95\linewidth]{"Sobol_MVN_guaranteed_time_MLE__d2_r1_2019-May-29"}
	\caption[Guaranteed:]{\code{cubBayesNet\_g}: Multivariate normal probability example using the empirical Bayes stopping criterion.}
...
\begin{figure}
	...
	\caption[Keister guaranteed:MLE]{\code{cubBayesNet\_g}: Keister example using the empirical Bayes stopping criterion.}
...
\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"Sobol_optPrice_guaranteed_time_MLE__d12_r1_2019-May-30"}
...
We demonstrated the capability our new Bayesian cubature algorithm to successfully compute the integrals faster within the user defined error tolerances. But there are few potential improvements and new areas of applications.
